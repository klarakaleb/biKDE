i! Bivariate Kernel Density Estimation
!
  ...
  real, parameter :: PI = 3.14159265358979323846264338327950288419716939937510582097494459 ! from https://oeis.org/A000796
   ...
   subroutine get_biKDE(data, incr, btype, kernel, g_array)
    ...
    ! basi workflow
   	...
   	! INPUT
 		real, intent(in) :: incr
 		character(len=3) :: btype !  'S' only supported for now
   ...
   subroutine make_grid_array(data, incr, g_array) 
      ...
      xsize = ceiling((maxx-minx)/incr) + 4  ! padding
      ysize = ceiling((maxy-miny)/incr) + 4  ! padding
      ...
      do while (value.le.(maxx+2*incr).and.i.le.xsize)
        grid_x(i) = value
        value = value + incr
        i = i + 1
      end do
      ...
      do while (value.le.(maxy+2*incr).and.i.le.ysize)
        grid_y(i) = value
        value = value + incr
        i = i + 1
      end do
   ...
   subroutine density(data,n,bandwidth,g_array,PI,kernel)
    ...
    subroutine get_log_fraction(v_coordinates, g_array, log_fraction)
